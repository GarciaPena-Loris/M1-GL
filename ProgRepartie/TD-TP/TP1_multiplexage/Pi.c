...
int initialisation(char *adresseIPPconfig, char *portPconfig, struct sockaddr_in structAdresseServeurTCP, int numeroPi, int socketPiTCP, int *tabSocketsVoisins)
{
    ...
    printf("\033[0;%dm[%d] ✅ Pi : Creation de la 🧦 Pi réussie.\033[0;30m\n", (30+numeroPi), numeroPi);
    ...
    printf("\033[0;%dm[%d] 🏷️  Nommage de la socket réussi.\033[0;30m\n", (30+numeroPi), numeroPi);
    ...
    printf("\033[0;%dm[%d] 📨 Envois de son numéro (%d) au serveur %s:%s\033[0;30m\n", (30+numeroPi), numeroPi, numeroPi, adresseIPPconfig, portPconfig);
    ...
    printf("\033[0;%dm[%d]📩 Recevoir nombre Accept et de Connect\033[0;30m\n", (30+numeroPi), numeroPi);
    ...
    printf("\033[0;%dm[%d] \t 🔗 Nombre de Connect a faire : %d\033[0;30m\n", (30+numeroPi), numeroPi, compteurVoisins.nombreConnect);
    printf("\033[0;%dm[%d] \t 📥 Nombre d'Accept a faire : %d\033[0;30m\n", (30+numeroPi), numeroPi, compteurVoisins.nombreAccept);
    ...
    if (compteurVoisins.nombreConnect > 0)
        printf("\033[0;%dm[%d] 🔗 Reception et connexion des %d voisins\033[0;30m\n", (30+numeroPi), numeroPi, compteurVoisins.nombreConnect);
    ...
    for (int i = 0; i < compteurVoisins.nombreConnect; i++)
    {
        ...
        printf("\033[0;%dm[%d] \t🛰️  Connection du voisin %s:%d réussi.\033[0;30m\n", (30+numeroPi), numeroPi, ipVoisin, portVoisin);

        tabSocketsVoisins[i] = socketClientTCP;
    }

    if (compteurVoisins.nombreConnect > 0)
        printf("\033[0;%dm[%d] 🏆 Fin reception des 🧦 voisins --\033[0;30m\n", (30+numeroPi), numeroPi);

    // -- Etape 4 : Envois confirmation a Pconfig
    printf("\033[0;%dm[%d] ⏲️ Envois confirmation à Pconfig.\033[0;30m\n", (30+numeroPi), numeroPi);
    int conf = 1;
    ...
    printf("\033[0;%dm[%d] ⏳ Attente reception confirmation...\033[0;30m\n", (30+numeroPi), numeroPi);
    ...
    if (compteurVoisins.nombreAccept > 0)
        printf("\033[0;%dm[%d] 📥 Accepter les demandes des %d voisins\033[0;30m\n", (30+numeroPi), numeroPi, compteurVoisins.nombreAccept);
    ...
    for (int i = 0; i < compteurVoisins.nombreAccept; i++)
    {
        ...
        printf("\033[0;%dm[%d] \t👋 Voisin n°%d du Pi %d accepté.\033[0;30m\n", (30+numeroPi), numeroPi, numeroPi, i);
    }

    if (compteurVoisins.nombreAccept > 0)
        printf("\033[0;%dm[%d] 🏆 Fin d'acceptation des voisins\033[0;30m\n\033[0;30m\n", (30+numeroPi), numeroPi);

    printf("\033[0;%dm[%d] 🥳🎉🎉🎉 Tous les voisins sont connectée !! 🎉🎉🎉🥳\033[0;30m\n\033[0;30m\n", (30+numeroPi), numeroPi);
...
void *diffusion_message(void *params)
{
    ...
    printf("\033[0;%dm[%d] --- 💬 Envois du message au voisin n°%d ---\033[0;30m\n", (30+numeroPi), numeroPi, idThread);
    printf("\033[0;%dm[%d] -- 📏 Envoi de la taille du message --\033[0;30m\n", (30+numeroPi), numeroPi);
    ...
    if (resSendTCPsize == 0 || resSendTCPsize == -1) {
        printf("\033[0;%dm[%d] ❌ Pi : Probleme lors du sendTCP.\033[0;30m\n", (30+numeroPi), numeroPi);
    }

    printf("\033[0;%dm[%d] \tMessage envoyé : '%d'\033[0;30m\n", (30+numeroPi), numeroPi, tailleMessage);

    printf("\033[0;%dm[%d] -- 💬 Envois du message  --\033[0;30m\n", (30+numeroPi), numeroPi);
    ssize_t resSendTCP = sendTCP(socketVoisin, message, sizeof(message));
    if (resSendTCP == 0 || resSendTCP == -1) {
        printf("\033[0;%dm[%d] ❌ Pi : Probleme lors du sendTCP.\033[0;30m\n", (30+numeroPi), numeroPi);
    }
        printf("\033[0;%dm[%d] \tMessage envoyé : '%s'\033[0;30m\n", (30+numeroPi), numeroPi, message);

    printf("\033[0;%dm[%d] --- 🏆 Fin envoie du message au voisin n°%d ---\033[0;30m\n", (30+numeroPi), numeroPi, idThread);
}

void messageMultiplexe(int numeroPi, int *tabSocketsVoisins, int nombreVoisins, int intervaleTemps)
{

    printf("\033[0;%dm[%d] ----- 📨 Envois d'un message aux %d voisins du Pi n°%d toutes les %d sec -----\033[0;30m\n", (30+numeroPi), numeroPi, nombreVoisins, numeroPi, intervaleTemps);
    ...
    printf("\033[0;%dm[%d] ✅ Fin d'affichage des voisins.\033[0;30m\n", (30+numeroPi), numeroPi);
}
...
int main(int argc, char *argv[])
{
    ...
    printf("\033[0;%dm[%d] 🏁 Fin du Pi n°%d !\033[0;30m\n", (30+numeroPi), numeroPi, numeroPi);
