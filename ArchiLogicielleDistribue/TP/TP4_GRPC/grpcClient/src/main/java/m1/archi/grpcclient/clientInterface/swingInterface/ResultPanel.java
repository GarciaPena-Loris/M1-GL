...
import m1.archi.grpcclient.models.hotelModels.Chambre;
import m1.archi.grpcclient.models.hotelModels.Hotel;
import m1.archi.grpcclient.models.hotelModels.Offre;
import m1.archi.proto.services.ComparateurServiceGrpc;
import m1.archi.proto.services.GetAgenceByIdRequest;
import m1.archi.proto.services.GetHotelAgenceByIdRequest;
...
public class ResultPanel extends JPanel {
    ...
    public ResultPanel(CustomContentPane customContentPane, Map<Long, List<List<Offre>>> mapAgenceOffres, ComparateurServiceGrpc.ComparateurServiceBlockingStub proxyComparateur) {
        ...
        for (Map.Entry<Long, List<List<Offre>>> entry : mapAgenceOffres.entrySet()) { // Pour chaque agence
            ...
            for (List<Offre> offreList : offres) { // Pour chaque hôtel
                for (Offre offre : offreList) { // Pour chaque offre
                    if (mapOffreHotelPrixMin.containsKey(offre.getIdHotel())) {
                        if (mapOffreHotelPrixMin.get(offre.getIdHotel()).getPrixAvecReduction() > offre.getPrixAvecReduction()) {
                            mapOffreHotelPrixMin.put(offre.getIdHotel(), offre);
                        }
                    } else {
                        mapOffreHotelPrixMin.put(offre.getIdHotel(), offre);
                    }
    ...
    private JPanel createAgencePanel(Long idAgence, List<List<Offre>> offres) {
        ...
        // Recupere l'agence associé
        try {
            GetAgenceByIdRequest getAgenceByIdRequest = GetAgenceByIdRequest.newBuilder()
                    .setIdAgence(idAgence)
                    .build();
            Agence agence = new Agence(proxyComparateur.getAgenceById(getAgenceByIdRequest));
            ...
            for (List<Offre> offreList : offres) { // Pour chaque hôtel
                GetHotelAgenceByIdRequest getHotelAgenceByIdRequest = GetHotelAgenceByIdRequest.newBuilder()
                        .setIdHotel(offreList.getFirst().getIdHotel())
                        .setIdAgence(idAgence)
                        .build();
                Hotel hotel = new Hotel(proxyComparateur.getHotelById(getHotelAgenceByIdRequest));
            ...
            System.out.println("Erreur lors de la récupération de l'agence");
        }
    ...
    private JPanel createOffrePanel(Agence agence, Offre offre, Color hotelColor) {
        ...
        else if (offre.getIdOffre() == mapOffreHotelPrixMin.get(offre.getIdHotel()).getIdOffre())
        ...
        // Calcul du nombre de nuit a partir des date en timestamp
