...
public class RandomDonneesAgence {
    ...
    public static Agence randomAgence() {
        return new Agence(null, randomNomAgence(), new HashMap<>(), new ArrayList<>());
    }
    ...
    public static List<Long> randomHotelPartenaire(List<Long> listeIdHotel) {
        // Recupere aleatoirement 'nombreHotelPartenaire' hotels de la liste 'listeHotel'
        List<Long> listeHotelPartenaire = new ArrayList<>();
        for (int i = 0; i < randomNombreHotelPartenaire(listeIdHotel.size()); i++) {
            Long hotel = getElementListeAleatoire(listeIdHotel.toArray(new Long[0]));
            if (!listeHotelPartenaire.contains(hotel)) {
                listeHotelPartenaire.add(hotel);
            }
        }
    ...
    public static Map<Long, Integer> randomReductionHotelPartenaire(List<Long> listeHotel) {
        Map<Long, Integer> mapReductionHotelPartenaire = new HashMap<>();
        for (Long idHotel : randomHotelPartenaire(listeHotel)) {
            mapReductionHotelPartenaire.put(idHotel, randomReduction());
        }
